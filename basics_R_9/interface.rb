...
class Interface
  ...
  # 2
  def train_new
    number = get_user_input("Введите номер поезда формате: три буквы или три цифры +'-'(может быть или не быть) + две буквы или цифры").downcase.to_s
  ...
  def add_route
    ...
    puts "Маршрут #{name_route} добавлен к поезду #{number}"
  rescue StandardError
  ...
  def list_station_list_trins_at_station
    puts %(
          ...
          3. Просмотр списка вагонов поезда.
          4. Просмотр списка станций пройденных поездом. 
      )
